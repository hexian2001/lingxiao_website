...
<div class="doc-flow" role="img" aria-label="MCP Forge 生成管线：需求、模板选择、代码生成、沙箱冒烟、Inspector 校验、注册。">
  <span>需求</span><i>→</i><span>模板选择</span><i>→</i><span>代码生成</span><i>→</i><span>沙箱冒烟</span><i>→</i><span>Inspector 校验</span><i>→</i><strong>注册</strong>
</div>
