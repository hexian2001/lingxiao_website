...
<div class="doc-decision-flow" role="img" aria-label="Eternal 状态流转：IDLE→CHECK→有问题?→PATROL→THINK→需要行动?→执行。分支：↘无异常→WAIT→IDLE退避，↘不需要行动→WAIT。">
  <div class="dc-row">
    <span>IDLE (30s)</span><i> → </i><span>CHECK</span><i> → </i><span>有问题？</span><i> → </i><span>PATROL</span><i> → </i><span>THINK</span><i> → </i><span>需要行动？</span><i> → </i><strong>执行</strong>
  </div>
  <div class="dc-row">
    <i class="dc-fork">↘</i><span>无异常</span><i> → </i><span>WAIT</span><i> → </i><span>IDLE（退避增长）</span>
    <i class="dc-fork dc-fork-2">↘</i><span>不需要行动</span><i> → </i><span>WAIT</span>
  </div>
</div>
